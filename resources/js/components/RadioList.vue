<template>
    <ul class="radio-list__list">
        <li class="radio-list__list-item" v-for="option in options" :key="option.value">
            ...
            <RadioList :options="option.children" :attribute="attribute" v-if="option.children.length" v-show="isExpanded(option.value)" @change="handleChange" :selected="selected" @optionSelected="onChildListOptionSelected"></RadioList>
        </li>
    </ul>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    onChildListOptionSelected(optionId) {
      this.expandOption(optionId);
      var firstOption = this.options[0];
      if (isNil(firstOption.parent_id)) {
        return;
      }
